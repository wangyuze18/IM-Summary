...
两种模式共用“重要消息”模型配置与相同原始输入，避免用更强的抽取模型制造不公平提升。团队模式的预期收益来自证据组织和审核闭环，实际提升以同一数据集上的六项指标为准。审核失败只回退对应任务，默认允许 6 轮定向返工；每轮会把具体字段/消息、问题描述和修订建议传回生成器，已通过的任务不重复生成。
...
六张指标卡始终保持可见，并作为不可收缩的独立面板参与内容区滚动：当前摘要已有评测时展示对应分数；有黄金摘要但尚未评测时使用“—”占位并提供“开始评测”；没有黄金摘要时仍显示六张占位卡，并提示先导入黄金摘要。页面不会用其他摘要版本的历史分数替代当前结果。

导入预览、重新分析、删除确认、模型设置、成员列表、组织关系图和评测详情统一使用论文卡片式弹层；删除操作不调用浏览器原生确认框，导出菜单和顶部通知也沿用同一视觉语言。
