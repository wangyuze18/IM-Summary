...
- 摘要评测输入会剥离重要消息；文本相似度本地计算，单次评审模型调用产出准确率、遗漏率与 `llm_score`，并限制为短 JSON 输出。
- 重要消息按稳定 `messageId` 在本地计算精确率和召回率，不再发起第二次评审模型调用。
- 导入样例未提供黄金重要消息时，两项重要消息指标为空值。
- 同一摘要与同一黄金版本只执行一次评测，重复请求直接返回已有有效记录。
- 模型目录接口同时返回模型列表和思考能力；自动配置只需一次目录请求，完整生成验证保留为用户主动操作。
