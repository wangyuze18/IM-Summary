...
/**
 * Agent Prompt 模板。
 * 对齐 docs/prompt-strategy/00_Prompt设计总纲.md 与 02_单模型基础模式Prompt.md。
 */
public final class PromptTemplates {
    ...
    public static final String SUMMARY_SYSTEM = GLOBAL_RULES + """
            ...
            6. 存在目标用户时输出 personalHighlights（1-3 条），说明"为什么与该用户相关"。
            数组允许为空，禁止为凑条数补造内容。
            ...
            ## 输出 JSON 格式
            {
              ...
              "openIssues":["问题"],
              "keyInfo":["关键信息"],
              "personalHighlights":[{"content":"重点","reason":"相关性原因","priority":"高"}]
            }
            """;
    ...
    public static final String PERSONALIZATION_AUDITOR_SYSTEM = GLOBAL_RULES + """
            ...
            检查个性化重点与重要性解释是否符合 User Context Card。检查项：
            ...
            ## 输出 JSON 格式
            {
              ...
              "issues":[{"type":"grounding|over_personalization|coverage|boundary",
                "severity":"error|warning","fieldPath":"personalHighlights[0]",
                "description":"问题描述","routeTo":"summary|relevance|user_context"}]
            }
            无问题时 passed 为 true 且 issues 为空数组。
            """;

    /** 单模型基础模式：输出约束 + 简单提示词（对齐参考实现风格，基线对照用） */
    public static final String SINGLE_MODEL_SYSTEM = """
            ...
            6. 若提供目标用户，输出 personalHighlights（1-3 条）说明与其相关的内容及原因。
            ...
            仅输出一个 JSON 对象，不要输出 markdown 代码块标记，不要解释：
            {
              ...
              "openIssues":["问题"],
              "keyInfo":["关键信息"],
              "personalHighlights":[{"content":"重点","reason":"原因","priority":"高"}]
            }
            缺失字段用空数组，不要省略字段。
            """;
    ...
    public static final String EVALUATION_JUDGE_SYSTEM = """
            ...
            - recall（0-1）：黄金摘要中的关键信息点被生成摘要覆盖的比例。
            - keyInformationOmissionRate（0-1）：黄金摘要中的关键信息点被遗漏的比例（= 1 - recall 的关键信息口径，关注截止日期、负责人、版本号等硬信息）。

            仅输出 JSON：{"accuracy":0.9,"recall":0.85,"keyInformationOmissionRate":0.1}
            """;
