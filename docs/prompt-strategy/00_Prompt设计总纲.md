...
摘要生成强调固定结构、短句、决议/待办边界和实体保真。重要消息强调选择单条原始消息、说话人本名、来源保真、去重和状态更新。完整要求见 [基础模式 Prompt](02_单模型模式Prompt.md)。
...
纳入范围：待办、决议、风险、审批、进度、阻断及其他有明确业务价值的消息。每一条必须对应一个原始 `messageId`，`content` 必须保留原文，不可摘要改写。
...
`speaker` 必须等于原消息的 `senderDisplayName`，不得添加 `@`、职位或角色；每条只允许上述四个字段。
...
审核重要消息的假阳性、遗漏、来源保真和过时状态。问题类型为 `false_positive`、`omission`、`source`、`state` 或 `schema`。

审核结果将“会改变条目取舍”与“不影响取舍的表达提醒”分开：明确待办、决议、风险、审批、阻断或关键进度的遗漏，以及误收闲聊、原文错配、过时状态，均为 `error` 并必须返回定向修订；`warning` 仅用于次要上下文表达差异。每条问题必须同时给出可定位的 `description` 与可执行的 `suggestion`，避免生成器收到无法行动的笼统批评。

## 5. 定向修订 Prompt

修订输入由三部分组成：当前分支结果、该分支审核问题、原始证据/共享账本。问题会被压缩为字段/消息定位、缺陷描述和修订建议，系统明确要求只修复列出的问题并返回完整 JSON。默认允许 6 轮返工，仅把它作为防止模型死循环的保险丝，已通过的分支不会重复调用。
