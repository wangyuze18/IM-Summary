...
- 六张指标卡始终显示且不会被内容区的纵向弹性布局压缩；当前版本有黄金摘要但无评测记录时使用“—”占位并提供“开始评测”，没有黄金摘要时保留占位卡并提示先导入标注，切换摘要版本时只显示该版本对应的指标。
- 导入、重新分析、删除、模型设置、成员、组织关系和评测详情统一为论文卡片式弹层；删除确认不使用浏览器原生弹窗，导出菜单与顶部通知使用同一套边框、色彩和动效。
- 审核未通过会点亮对应回退箭头；审核已通过但留有非阻断问题时显示“有提醒”，不伪装成返工。
- 页面不展示账户个人、目标用户或个人关注项，也不显示解释性状态口号。
- 原始消息中的 `@所有人` 与普通 @ 提及一样高亮，但不绑定到某个成员账户。
- 所有视觉滚动条保持隐藏，滚轮、触控板、键盘和横向滚动能力不受影响。
- 群组信息与组织关系在桌面布局中严格等宽；“共 N 人”保持完整显示，群名在单行空间不足时自然转入下一行，不提前截断。
