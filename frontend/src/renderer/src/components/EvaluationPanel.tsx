...
interface Props { groupName: string; records: EvaluationRecord[]; currentMetrics: EvaluationMetrics | null; evaluating: boolean; onEvaluate: () => void; onToast: (text: string) => void }
...
export default function EvaluationPanel({ groupName, records, currentMetrics, evaluating, onEvaluate, onToast }: Props) {
  ...
  return <section className={`panel evaluation-panel ${evaluating ? 'is-evaluating' : ''}`}>
    ...
    {!currentMetrics && !evaluating && <div className="evaluation-empty"><span>当前版本尚未评测</span><button className="btn small" onClick={onEvaluate}>开始评测</button></div>}
