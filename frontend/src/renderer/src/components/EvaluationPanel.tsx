...
const MODE_LABEL: Record<AnalysisMode, string> = { 'agent-workflow': '团队工作流', 'single-model': '单模型' }
...
export default function EvaluationPanel({ groupName, records, currentMetrics, onToast }: Props) {
  ...
  return <section className="panel">
    <div className="panel-header"><div className="panel-title">双任务评测指标（基于黄金标注）</div><button className="link-more" onClick={() => setCollapsed((v) => !v)}>{collapsed ? '展开评测历史 ▾' : '收起评测历史 ▴'}</button></div>
