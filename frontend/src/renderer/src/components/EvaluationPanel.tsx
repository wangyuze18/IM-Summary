...
interface Props { groupName: string; records: EvaluationRecord[]; currentMetrics: EvaluationMetrics | null; evaluating: boolean; evaluable: boolean; onEvaluate: () => void; onToast: (text: string) => void }
...
function MetricsCards({ metrics }: { metrics: EvaluationMetrics | null }) {
  const names = ['摘要准确率', '关键信息遗漏率', '文本相似度', '大模型文本评分', '重要消息精确率', '重要消息召回率']
  const values = metrics
    ? [pct(metrics.accuracy), pct(metrics.keyInformationOmissionRate), metrics.rougeL.toFixed(2), metrics.llmScore?.toFixed(1) ?? '—', optionalPct(metrics.importantMessagePrecision), optionalPct(metrics.importantMessageRecall)]
    : names.map(() => '—')
  return <div className={`metrics-cards ${metrics ? '' : 'metrics-pending'}`}>{names.map((name, index) => <div className="metric-card" key={name}><div className="v">{values[index]}</div><div className="n">{name}</div></div>)}</div>
}
...
export default function EvaluationPanel({ groupName, records, currentMetrics, evaluating, evaluable, onEvaluate, onToast }: Props) {
  ...
  return <section className={`panel evaluation-panel ${evaluating ? 'is-evaluating' : ''}`}>
    ...
    {!currentMetrics && !evaluating && <div className="evaluation-empty"><span>{evaluable ? '当前版本尚未评测' : '导入黄金摘要后可进行评测'}</span>{evaluable && <button className="btn small" onClick={onEvaluate}>开始评测</button>}</div>}
    ...
    {detail && <PaperDialog title="评测详情" subtitle={`${detail.evaluatedAt} · ${MODE_LABEL[detail.mode]} · v${detail.summaryVersion}`} size="md" onClose={() => setDetail(null)}>
      <div className="evaluation-detail-grid">
        {[
          ['摘要准确率', pct(detail.metrics.accuracy)],
          ['关键信息遗漏率', pct(detail.metrics.keyInformationOmissionRate)],
          ['文本相似度', detail.metrics.rougeL.toFixed(2)],
          ['大模型文本评分', detail.metrics.llmScore?.toFixed(1) ?? '—'],
          ['重要消息精确率', optionalPct(detail.metrics.importantMessagePrecision)],
          ['重要消息召回率', optionalPct(detail.metrics.importantMessageRecall)]
        ].map(([name, value]) => <div className="evaluation-detail-item" key={name}><span>{name}</span><b>{value}</b></div>)}
      </div>
    </PaperDialog>}
  </section>
}
